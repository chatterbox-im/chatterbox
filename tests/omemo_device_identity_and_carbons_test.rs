...
async fn test_omemo_device_identity_and_carbons() -> Result<()> {
    ...
    match ca_client.enable_carbons().await {
        ...
        Err(e) => {
            warn!(
                "Failed to enable message carbons: {}, but continuing test",
                e
            );
        ...
        .await
    {
        ...
        Err(e) => {
            error!(
                "Failed to connect to XMPP server as ca (second device): {}",
                e
            );
            let _ = timeout(TokioDuration::from_secs(5), ca_client.disconnect()).await;
            return Err(anyhow::anyhow!(
                "Failed to connect as ca (second device): {}",
                e
            ));
        }
    }
    ...
    match ca_device2_client.initialize_client().await {
        ...
        Err(e) => {
            ...
            return Err(anyhow::anyhow!(
                "Failed to initialize OMEMO for second device: {}",
                e
            ));
        }
    }
    ...
    match ca_device2_client.enable_carbons().await {
        ...
        Err(e) => {
            warn!(
                "Failed to enable message carbons on second device: {}, but continuing test",
                e
            );
            // Continue with the test even if enabling carbons fails
        }
    ...
    let test_message = format!(
        "OMEMO test message with carbons - {}",
        chrono::Utc::now().timestamp()
    );
    info!(
        "Sending OMEMO encrypted message from first device to {}: {}",
        cb_jid, test_message
    );

    match ca_client
        .send_encrypted_message(cb_jid, &test_message)
        .await
    {
        Ok(_) => info!("OMEMO encrypted message sent successfully from first device"),
        Err(e) => {
            error!(
                "Failed to send OMEMO encrypted message from first device: {}",
                e
            );
            let _ = timeout(TokioDuration::from_secs(5), ca_client.disconnect()).await;
            let _ = timeout(TokioDuration::from_secs(5), ca_device2_client.disconnect()).await;
    ...
    let message_received =
        match wait_for_message(&mut cb_msg_rx, |msg| msg.content == test_message, 5).await {
            Ok(msg) => {
