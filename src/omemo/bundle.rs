...
impl super::OmemoManager {
    ...
    pub async fn publish_bundle(&self, bundle: OmemoBundle) -> Result<()> {
        ...
        match self
            .publish_pubsub_item(None, &node_name, item_id, &bundle_xml)
            .await
        {
            Ok(_) => {
                info!(
                    "Bundle published successfully for device {}",
                    self.device_id
                );
                return Ok(());
            }
            Err(e) => {
                ...
                if e.to_string().contains("invalid-item") || e.to_string().contains("bad-request") {
                    ...
                    match self
                        .pubsub
                        .publish_item_alternative(None, &node_name, item_id, &bundle_xml)
                        .await
                    {
                        Ok(_) => {
                        ...
                        Err(alt_err) => {
                            ...
                            println!(
                                "[OMEMO ERROR] Failed to publish bundle (both methods): {}",
                                alt_err
                            );
                            return Err(anyhow!(
                                "Failed to publish bundle (both methods): {}",
                                alt_err
                            ));
                        }
                    }
    ...
    pub fn bundle_to_xml(&self, bundle: &OmemoBundle) -> Result<String> {
        ...
        let identity_key_b64 = base64::engine::general_purpose::STANDARD
            .encode(encode_public_key_with_prefix(&bundle.identity_key));
    ...
    pub fn parse_device_bundle_response(
        &self,
        response: &str,
        device_id: DeviceId,
        ...
        let document = roxmltree::Document::parse(response).map_err(|e| {
            warn!(
                "Failed to parse device bundle response for device {}: {}",
                device_id, e
            );
        ...
        if let Some(error) = document.descendants().find(|n| n.has_tag_name("error")) {
            ...
            for child in error.children() {
                if child.is_element() {
                    ...
                    } else if child.tag_name().namespace()
                        == Some("urn:ietf:params:xml:ns:xmpp-stanzas")
                    {
                        error_condition = child.tag_name().name();
                        break;
                    } else if ["item-not-found", "forbidden", "not-allowed"]
                        .contains(&child.tag_name().name())
                    {
                        error_condition = child.tag_name().name();
                        break;
                    }
            ...
            warn!(
                "Received error response for bundle request: type={}, condition={}",
        ...
        if items_element.is_none() {
            debug!(
                "Document structure for device {}: root={}, children={:?}",
                device_id,
        ...
        let item_element = items_element
            .unwrap()
            .descendants()
            .find(|n| n.has_tag_name("item"));
        if item_element.is_none() {
            debug!(
                "Items element children for device {}: {:?}",
                device_id,
                items_element
                    .unwrap()
                    .descendants()
        ...
        let bundle_elem = item_element
            .unwrap()
            .descendants()
            .find(|n| {
                n.has_tag_name("bundle")
                    && (n.has_attribute("xmlns") && n.attribute("xmlns") == Some(OMEMO_NAMESPACE))
            })
            .or_else(|| {
                item_element
                    .unwrap()
        ...
        let identity_key = bundle_elem
            ...
            .and_then(|encoded| {
                base64::engine::general_purpose::STANDARD
                    .decode(encoded)
                    .map_err(|e| {
                        OmemoError::ProtocolError(format!("Failed to decode identity key: {}", e))
                    })
            })?;
        let signed_pre_key_elem = bundle_elem
            .children()
        ...
        let signed_pre_key_id = signed_pre_key_elem
            ...
            .and_then(|id_str| {
                id_str.parse::<u32>().map_err(|e| {
                    OmemoError::ProtocolError(format!("Invalid signed pre-key ID: {}", e))
                })
            })?;
        let signed_pre_key = signed_pre_key_elem
            ...
            .and_then(|encoded| {
                base64::engine::general_purpose::STANDARD
                    .decode(encoded)
                    .map_err(|e| {
                        OmemoError::ProtocolError(format!("Failed to decode signed pre-key: {}", e))
                    })
            })?;
        let signature = bundle_elem
            ...
            .and_then(|encoded| {
                base64::engine::general_purpose::STANDARD
                    .decode(encoded)
                    .map_err(|e| {
                        OmemoError::ProtocolError(format!("Failed to decode signature: {}", e))
                    })
            })?;
        let mut pre_keys = Vec::new();
        if let Some(prekeys_elem) = bundle_elem.children().find(|n| n.has_tag_name("prekeys")) {
            for prekey_elem in prekeys_elem
                .children()
                .filter(|n| n.has_tag_name("preKeyPublic"))
            {
                let prekey_id = prekey_elem
                    .attribute("preKeyId")
                    .ok_or_else(|| OmemoError::ProtocolError("Pre-key ID not found".to_string()))
                    .and_then(|id_str| {
                        id_str.parse::<u32>().map_err(|e| {
                            OmemoError::ProtocolError(format!("Invalid pre-key ID: {}", e))
                        })
                    })?;
                let prekey = prekey_elem
                    .text()
                    .filter(|s| !s.is_empty())
    ...
    pub async fn request_pubsub_items(
        ...
        jid: &str,
        node: &str,
    ) -> Result<String, super::OmemoError> {
        ...
        match self.pubsub.request_items(jid, node).await {
            ...
            Err(e) => {
                warn!(
                    "Error in PubSub request to {}: {}, using fallback response",
                    jid, e
                );
                ...
                Err(super::OmemoError::ProtocolError(format!(
                    "Failed to make PubSub request: {}",
                    e
                )))
            }
        }
    ...
    pub fn validate_bundle_xml(xml: &str) -> Result<()> {
        // Parse the XML
        let doc =
            roxmltree::Document::parse(xml).map_err(|e| anyhow!("Invalid XML structure: {}", e))?;
        ...
        if root.tag_name().name() != "bundle" {
            return Err(anyhow!(
                "Root element must be 'bundle', found '{}'",
                root.tag_name().name()
            ));
        }
        ...
        if namespace != Some(OMEMO_NAMESPACE) {
            return Err(anyhow!(
                "Invalid namespace: expected '{}', found '{:?}'",
                OMEMO_NAMESPACE,
                namespace
            ));
        }
        ...
        for child_name in &required_children {
            if !root
                .children()
                .any(|child| child.is_element() && child.tag_name().name() == *child_name)
            {
