...
impl OmemoManager {
    ...
    pub(crate) async fn initialize_key_bundle_with_persistent_identity(&mut self) -> Result<(), OmemoError> {
    ...
    pub(crate) async fn get_device_ids_with_force_refresh(&self, jid: &str, force_refresh: bool) -> Result<Vec<DeviceId>, OmemoError> {
    ...
    pub(crate) async fn get_device_failure_count(&self, remote_jid: &str, remote_device_id: u32) -> u32 {
        let storage_guard = self.storage.lock().await;
    ...
    pub async fn trust_device_identity(&self, sender: &str, device_id: DeviceId) -> Result<(), OmemoError> {
        ...
        let storage_guard = self.storage.lock().await;
        storage_guard.set_trust_level(sender, device_id, TrustLevel::Trusted)
    ...
    pub async fn verify_device_identity(&self, sender: &str, device_id: DeviceId) -> Result<(), OmemoError> {
        ...
        let storage_guard = self.storage.lock().await;
        storage_guard.set_trust_level(sender, device_id, TrustLevel::Verified)
    ...
    pub async fn untrust_device_identity(&self, sender: &str, device_id: DeviceId) -> Result<(), OmemoError> {
        ...
        let storage_guard = self.storage.lock().await;
        storage_guard.set_trust_level(sender, device_id, TrustLevel::Untrusted)
    ...
    pub(crate) fn parse_device_list_response(&self, response: &str) -> Result<Vec<u32>, OmemoError> {
