...
fn generate_and_save_device_id() -> Result<DeviceId> {
    ...
    info!(
        "Generating new device ID: {} and saving to {}",
        device_id,
        path.display()
    );
...
fn deserialize_key_pair(serialized: &str) -> Result<KeyPair> {
    ...
    if parts.len() != 2 {
        return Err(anyhow!(
            "Invalid key pair format, expected 'public:private'"
        ));
    }
...
fn generate_and_save_identity_key() -> Result<KeyPair> {
    ...
    info!(
        "Generating new Identity Key and saving to {}",
        path.display()
    );
    ...
    if path.exists() {
        match load_multi_device_info(&path) {
            Ok(mut info) => {
                ...
                info!(
                    "Loaded existing multi-device info with {} known devices",
            ...
            Err(e) => {
                warn!(
                    "Failed to load multi-device info: {}, initializing new one",
                    e
                );
...
pub fn update_known_devices(device_ids: &[DeviceId]) -> Result<MultiDeviceInfo> {
    ...
    if added > 0 {
        ...
        save_multi_device_info(&path, &info)?;
        info!(
            "Added {} new device IDs, now tracking {} devices",
            added,
            info.known_device_ids.len()
        );
    } else {
...
mod tests {
    ...
    fn test_device_id_generation() {
        ...
        assert_ne!(
            ...
            "Two different generated IDs should be different"
        );
    }
    ...
    fn test_device_id_persistence() -> Result<()> {
        ...
        assert!(
            ...
            "Device ID file should exist after saving"
        );
    ...
    fn test_identity_key_generation() -> Result<()> {
        ...
        // The probability of these being equal is extremely small
        assert_ne!(
            key_pair.public_key, key_pair2.public_key,
            "Two different generated public keys should be different"
        );
        assert_ne!(
            key_pair.private_key, key_pair2.private_key,
            "Two different generated private keys should be different"
        );
    ...
    fn test_identity_key_serialization() -> Result<()> {
        ...
        assert_eq!(
            key_pair.public_key, deserialized.public_key,
            "Public key should be preserved through serialization"
        );
        assert_eq!(
            key_pair.private_key, deserialized.private_key,
            "Private key should be preserved through serialization"
        );
    ...
    fn test_identity_key_persistence() -> Result<()> {
        ...
        assert!(
            ...
            "Identity key file should exist after saving"
        );
        ...
        assert!(
            !was_generated,
            "Second call should load the existing identity key"
        );
        assert_eq!(
            key_pair.public_key, loaded_key.public_key,
            "Public key should be preserved when loaded"
        );
        assert_eq!(
            key_pair.private_key, loaded_key.private_key,
            "Private key should be preserved when loaded"
        );
    ...
    fn test_update_known_devices() -> Result<()> {
        ...
        println!(
            "[test_update_known_devices] file exists after save: {}",
            path.exists()
        );
        ...
        println!(
            "[test_update_known_devices] initial_info: {:?}",
        ...
        println!(
            "[test_update_known_devices] file exists before update: {}",
            path.exists()
        );
        ...
        println!(
            "[test_update_known_devices] loaded_info: {:?}",
            loaded_info.known_device_ids
        );
    ...
    fn test_remove_device() -> Result<()> {
        ...
        println!(
            "[test_remove_device] file exists after save: {}",
            path.exists()
        );
        ...
        let initial_info = load_multi_device_info(&path)?;
        println!(
            "[test_remove_device] initial_info: {:?}",
            initial_info.known_device_ids
        );
        assert!(
        ...
        let loaded_info = load_multi_device_info(&path)?;
        println!(
            "[test_remove_device] loaded_info after manual removal: {:?}",
            loaded_info.known_device_ids
        );
