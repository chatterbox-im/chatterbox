...
pub async fn fetch_device_list_with_fallbacks(
    jid: &str,
    pubsub: &dyn crate::omemo::OmemoPubSub,
) -> Result<Vec<DeviceId>, OmemoError> {
    info!(
        "[OMEMO] Attempting to fetch OMEMO device list for {} with fallbacks",
        jid
    );
    ...
    if bare_jid != jid {
        info!(
            "[OMEMO] Normalized JID for device discovery: {} -> {}",
            jid, bare_jid
        );
    }
    ...
    for namespace in &OMEMO_NAMESPACES {
        ...
        for format in &NODE_FORMATS {
            ...
            debug!(
                "[OMEMO] Attempting to fetch device list with node: {}",
                node
            );
            ...
            match timeout(
                ...
                pubsub.request_items(bare_jid, &node),
            )
            .await
            {
                Ok(Ok(xml)) => {
                    ...
                    match parsed_devices {
                        Ok(devices) if !devices.is_empty() => {
                            info!(
                                "[OMEMO] Found {} devices using node {}",
                                devices.len(),
                                node
                            );
                            ...
                            info!(
                                "[OMEMO] Successfully found devices for {}: {:?}",
                                jid, combined_devices
                            );
                        ...
                        Err(e) => {
                            debug!(
                                "[OMEMO] Error parsing device list response from node {}: {}",
                                node, e
                            );
                ...
                Ok(Err(e)) => {
                    ...
                    if e.to_string().contains("feature-not-implemented") {
                        warn!(
                            "[OMEMO] Server {} doesn't support OMEMO (feature-not-implemented)",
                            jid
                        );
                        return Ok(Vec::new());
                    }
        ...
        if namespace_had_valid_response {
            debug!(
                "[OMEMO] Namespace {} had valid response but no devices, trying next namespace",
                namespace
            );
        }
    }
    ...
    if any_success {
        ...
        info!(
            "[OMEMO] Combined device list after trying all methods: {:?}",
            combined_devices
        );
        return Ok(combined_devices);
    }
    ...
    // As a last resort, try to fetch bundle information for common device IDs
    try_fetch_bundle_for_common_device_ids(jid, pubsub).await
}
...
async fn try_fetch_bundle_for_common_device_ids(
    ...
    if !found_devices.is_empty() {
        info!(
            "[OMEMO] Found devices by checking bundles directly: {:?}",
            found_devices
        );
    ...
    Err(OmemoError::NoDeviceError(format!(
        "No OMEMO devices found for {}",
        jid
    )))
}
