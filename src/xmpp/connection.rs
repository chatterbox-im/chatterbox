...
impl XMPPClient {
    pub async fn connect(&mut self, server: &str, username: &str, password: &str) -> Result<()> {
        ...
        for attempt in 1..=3 {
            info!(
                "Attempting to connect to XMPP server (attempt {}/3)...",
                attempt
            );
                ...
                .await
            {
                ...
                Ok(false) => {
                    ...
                    error!(
                        "Failed to connect on attempt {}/3: Connection handshake failed",
                        attempt
                    );
                    last_error = Some(err);
                }
        ...
        // All attempts failed
        let err = last_error
            .unwrap_or_else(|| anyhow!("Failed to connect to XMPP server after 3 attempts"));
